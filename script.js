// ---------- hero waybill: randomize route on each load ----------
(function () {
  var districts = [
    'Голосіївський', 'Дарницький', 'Деснянський', 'Дніпровський',
    'Оболонський', 'Печерський', 'Подільський', 'Святошинський',
    "Солом'янський", 'Шевченківський', 'Троєщина', 'Борщагівка'
  ];
  var fromEl = document.getElementById('route-from');
  var toEl = document.getElementById('route-to');
  if (fromEl && toEl) {
    var a = Math.floor(Math.random() * districts.length);
    var b;
    do { b = Math.floor(Math.random() * districts.length); } while (b === a);
    fromEl.textContent = districts[a];
    toEl.textContent = districts[b];
  }
})();
